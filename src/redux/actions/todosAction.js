...
export const getAllTodos = async (dispatch) => {
  ...
  try {
    const res = await axios.get("http://localhost:5000" + "/api/v1/todos-all");
...
export const createTodo = (payload) => async (dispatch) => {
  ...
  try {
    const res = await axios.post("http://localhost:5000" + "/api/v1/todo/new", {
...
export const deleteTodo = (payload) => async (dispatch) => {
  ...
  try {
    const res = await axios.delete("http://localhost:5000" + "/api/v1/todo/" + payload.todoId);
